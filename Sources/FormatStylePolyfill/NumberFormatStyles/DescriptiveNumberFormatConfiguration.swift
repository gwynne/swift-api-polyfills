...
public enum _polyfill_DescriptiveNumberFormatConfiguration {
    ...
    struct Collection: Codable, Hashable {
        ...
        var icuNumberFormatStyle: UNumberFormatStyle {
            ...
            case .spellOut, .cardinal: UNUM_SPELLOUT
            case .ordinal: UNUM_ORDINAL
            }
        }
    }
}
...
public struct _polyfill_FormatStyleCapitalizationContext: Codable, Hashable, Sendable {
    public static var unknown: Self {
        .init(UDISPCTX_CAPITALIZATION_NONE)
    }
    
    public static var standalone: Self {
        .init(UDISPCTX_CAPITALIZATION_FOR_STANDALONE)
    }
    
    public static var listItem: Self {
        .init(UDISPCTX_CAPITALIZATION_FOR_UI_LIST_OR_MENU)
    }
    
    public static var beginningOfSentence: Self {
        .init(UDISPCTX_CAPITALIZATION_FOR_BEGINNING_OF_SENTENCE)
    }
    
    public static var middleOfSentence: Self {
        .init(UDISPCTX_CAPITALIZATION_FOR_MIDDLE_OF_SENTENCE)
    }
    
    private init(_ icuContext: UDisplayContext) {
        self.icuContext = icuContext
    }
    ...
    public init(from decoder: any Decoder) throws {
        self.init(try .init(0x100 | ((5 - decoder.container(keyedBy: CodingKeys.self).decode(UInt32.self, forKey: .option)) % 5)))
    }
