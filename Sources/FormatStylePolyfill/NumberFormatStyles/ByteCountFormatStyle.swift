...
public struct _polyfill_ByteCountFormatStyle: _polyfill_FormatStyle, Sendable {
    ...
    enum Unit: Int, Hashable, Comparable, Strideable {
        ...
        var name: String {
            Self.unitNames[self.rawValue]
        }
        
        var decimalSize: Int64 {
            Self.decimalByteSizes[self.rawValue]
        }
        
        var binarySize: Int64 {
            Self.binaryByteSizes[self.rawValue]
        }
    ...
    public func format(_ value: Int64) -> String {
        .init(self.attributed.format(value).characters)
    }
    ...
    public struct Units: OptionSet, Codable, Hashable, Sendable {
        ...
        public init(rawValue: Int) {
            self.rawValue = (rawValue == 0 ? 0x0FFFF : rawValue)
        }
        ...
        /// The kilobytes unit.
        public static var kb: Self {
            .init(rawValue: 1 << 1)
        }
        
        /// The megabytes unit.
        public static var mb: Self {
            .init(rawValue: 1 << 2)
        }
        
        /// The gigabytes unit.
        public static var gb: Self {
            .init(rawValue: 1 << 3)
        }
        
        /// The terabytes unit.
        public static var tb: Self {
            .init(rawValue: 1 << 4)
        }
        
        /// The petabytes unit.
        public static var pb: Self {
            .init(rawValue: 1 << 5)
        }
        
        /// The exabytes unit.
        public static var eb: Self {
            .init(rawValue: 1 << 6)
        }
        
        /// The zettabytes unit.
        public static var zb: Self {
            .init(rawValue: 1 << 7)
        }
        
        /// A value that indicates a format style should express byte counts as yottabytes or higher.
        public static var ybOrHigher: Self {
            .init(rawValue: 0x0FF << 8)
        }
        ...
        fileprivate var smallestUnit: Unit {
            (Unit.byte ... .petabyte).first(where: {
                self.contains(.init(rawValue: $0.rawValue))
            }) ?? .petabyte
        }
    }
    ...
    public struct Attributed: _polyfill_FormatStyle, Sendable {
        ...
        func useSpelloutZero(forLocale locale: Foundation.Locale, unit: Unit) -> Bool {
            guard unit == .byte || unit == .kilobyte else {
                return false
            }
        ...
        private func formatImpl(_ value: ICUNumberFormatter.Value) -> Foundation.AttributedString {
            ...
            if self.spellsOutZero, value.isZero {
                ...
                guard self.useSpelloutZero(forLocale: locale, unit: unit) else {
            ...
            let bestUnit: Unit = {
                ...
                for (idx, size) in maxSizes.enumerated() {
                    guard self.allowedUnits.contains(.init(rawValue: 1 << idx)) else {
                        continue
                    }
            
                    bestUnit = Unit(rawValue: idx)!
                    if abs(value.doubleValue) < Double(size) {
                        break
                    }
                }
                return bestUnit
            ...
            var attributedString = ICUByteCountNumberFormatter.create(
                for: "\(precisionSkeleton) measure-unit/digital-\(bestUnit.name) \(bestUnit == .byte ? "unit-width-full-name" : "")",
                locale: self.locale
            )!.attributedFormat(
                .floatingPoint(value.doubleValue / Double(decimal ? bestUnit.decimalSize : bestUnit.binarySize)),
                unit: bestUnit
            )
            ...
            if self.includesActualByteCount {
                ...
                let localizedParens = ICU4Swift.localizedParenthesis(locale: self.locale)
        ...
        /// - Parameter value: The 64-bit byte count to format.
        /// - Returns: A formatted representation of `value`, formatted according to the style’s configuration.
        public func format(_ value: Int64) -> Foundation.AttributedString {
            self.formatImpl(.integer(value))
        }
    }
}
