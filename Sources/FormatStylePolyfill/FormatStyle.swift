...
public protocol _polyfill_FormatStyle: Codable, Hashable {
    /// The type this format style accepts as input.
    ///
    /// Swift type inference uses this value to determine which static accessors are available at a given call
    /// point. For example, when you format an `Int32`, you can use the static `number` property that provies a
    /// `IntegerFormatStyle<Int32>`, as seen in the following example. This works because the style’s input type
    /// `IntegerFormatStyle.FormatInput` is a `BinaryInteger` generically constrained to the `Int32` type.
    ///
    /// ```swift
    ...
    /// - Returns: A representation of `value`, in the `FormatOutput` type, formatted according to the
